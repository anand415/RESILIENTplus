function [ovrall_err_train,ovrall_err_test]=mccvtest(ss,errall,Ctrain,Ctest)
load ../vocvalppb.mat
d=vocvalppb;
...
%        length((Casc_err_test(Cascts==ii)))
    ...
    indvdl_err_train(ii)=norm(Casc_err_train(Casctr==ii))/length((Casc_err_train(Casctr==ii)))^(0.5);
        indvdl_err_test(ii)=norm(Casc_err_test(Cascts==ii))/length((Casc_err_test(Cascts==ii)))^(0.5);
end
