function [x1,errall_ga_mrae]=mccvtrain_mrae_AS(ii,Ctrain)
load Rmat_fwd.mat
load ../vocvalppb.mat
% rng default
C=Rmat_fwd;
...
% end
[x1{jj},fval1,exitflag,output,lambda,grad,hessian] = ...
fmincon(@(x) 100*mean(abs(Ctr{jj}*x'-dtr{jj})),ones(1,36),[],[],[],[],zeros(1,36),[],[],options);
tmm=fun(x1{jj});
