function [ovrall_err_train,ovrall_err_test]=mccvtest_msrae(ss,errall,Ctrain,Ctest)
load ../vocvalppb.mat
...
for ii=1:47
    indvdl_err_train(ii)=(nanmedian(abs(Casc_err_train(Casctr==ii))/d(ii))).^2;
%     length(Casc_err_test(Cascts==ii))
    indvdl_err_test(ii)=(nanmedian(abs(Casc_err_test(Cascts==ii))/d(ii))).^2;
    %
...
%
ovrall_err_train=mean(indvdl_err_train);
